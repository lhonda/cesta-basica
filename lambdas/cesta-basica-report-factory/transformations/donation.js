module.exports = data => {
  ...
  const transformedData = `${header}\n${data.map(e => `${e.donationId};${e.leaderLogin};${e.leaderName};${e.quantity};${e.vouchers.received};${e.vouchers.delivered};${e.vouchers.notDelivered};${e.site.name};${e.site.city};${e.site.state};${parseDate(e.receivedDate)}`).join('\n')}`
